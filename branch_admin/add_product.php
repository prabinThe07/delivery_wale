...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare('INSERT INTO products (name, description, quantity, location_id, status, branch_id, created_at, updated_at) 
                                  VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())');
            $stmt->execute([$name, $description, $quantity, $location_id, $status, $branch_id]);
...
<div class="container-fluid px-4">
    <div class="row mb-4">
        <div class="col-md-12">
            ...
            <p class="text-muted">Create a new product or item for delivery.</p>
        </div>
    </div>
            ...
            <form method="post" action="">
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" id="quantity" name="quantity" value="<?php echo isset($_POST['quantity']) ? htmlspecialchars($_POST['quantity']) : '1'; ?>" min="1" required>
                    </div>
                </div>
